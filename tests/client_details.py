...
def main():
    with tempfile.TemporaryDirectory(prefix='zimbr-details-') as temp:
        ...
        with (root / 'server.log').open('w') as log:
            ...
            def start():
                nonlocal proc
                proc = subprocess.Popen([str(BIN / 'client-probe'), '--control', '--data-dir', str(client), '--token-file', str(relay / 'token'), '--port', str(port)], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=log, text=True)
                views = queue.Queue()
                def read():
            ...
            def until(views, predicate):
                ...
                while time.monotonic() < deadline:
                    ...
                    assert token not in line, 'Credentials must never enter diagnostics'
