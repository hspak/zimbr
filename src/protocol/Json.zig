...
pub fn check(bytes: []const u8, max_bytes: usize, max_tokens: usize) !void {
    return checkImpl(bytes, max_bytes, max_tokens, true);
}
fn checkImpl(bytes: []const u8, max_bytes: usize, max_tokens: usize, comptime vectorize: bool) !void {
...
test "vector string scanning matches scalar bounds across escapes and tails" {
    ...
    for (0..4000) |_| {
        ...
        const expected: ?anyerror = if (checkImpl(bytes[0..len], bytes.len, tokens, false)) null else |err| err;
        const actual: ?anyerror = if (check(bytes[0..len], bytes.len, tokens)) null else |err| err;
    ...
    for (0..48) |prefix| {
        ...
        for (0..input.len + 1) |len| {
            const expected: ?anyerror = if (checkImpl(input[0..len], 256, 32, false)) null else |err| err;
            const actual: ?anyerror = if (check(input[0..len], 256, 32)) null else |err| err;
            try std.testing.expectEqual(expected, actual);
        }
    }
}
