...
pub fn decode(a: u.Allocator, body: []const u8) ![]const u8 {
    ...
    for (prefixes) |prefix| {
        ...
        if (std.mem.startsWith(u8, text, "\xff\xfe")) {
            ...
            defer a.free(units);
            for (units, 0..) |*unit, i| unit.* = std.mem.readInt(u16, text[2 + i * 2 ..][0..2], .little);
            const decoded = std.unicode.utf16LeToUtf8Alloc(a, units) catch return error.Malformed;
...
fn readLength(b: []const u8, pos: *usize) !usize {
    ...
    const n: usize = if (size == 2) std.mem.readInt(u16, b[pos.*..][0..2], .little) else std.mem.readInt(u32, b[pos.*..][0..4], .little);
    pos.* += size;
    return n;
}
test "typed string preserves multiline emoji without arbitrary byte extraction" {
    ...
    const text = "\nHello e\xcc\x81 👩‍💻\n";
    for (prefixes) |p| {
        const fixture = try std.mem.concat(a, u8, &.{ p, &.{text.len}, text, "\x86" });
