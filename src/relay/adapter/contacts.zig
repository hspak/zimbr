...
extern fn zr_contacts_email_key(
    value: [*:0]const u8,
    fold_local: c_int,
    out: [*]u8,
    capacity: usize,
...
pub const Index = struct {
    ...
    pub const InitError = u.Allocator.Error || error{InvalidContacts};
    pub fn init(a: u.Allocator, contacts: []const Contact) InitError!Index {
        ...
        for (contacts, 0..) |contact, i| {
            ...
            for (contact.phones) |phone| {
                if (phone.len > 300 or (!std.mem.startsWith(u8, phone, "phone:") and !std.mem.startsWith(
                    u8,
                    phone,
                    "exact:",
                ))) return error.InvalidContacts;
    ...
    fn insert(
        self: Index,
        a: u.Allocator,
        map: *std.StringHashMapUnmanaged(Candidate),
        key: []const u8,
        i: usize,
    ) !void {
        const entry = try map.getOrPut(a, key);
        if (!entry.found_existing) entry.value_ptr.* = .{ .index = i } else if (!u.eq(
            self.contacts[entry.value_ptr.index].id,
            self.contacts[i].id,
        )) entry.value_ptr.ambiguous = true;
    }
    pub const MatchError = u.Allocator.Error || error{InvalidContacts};
    pub fn match(self: Index, a: u.Allocator, address: []const u8, region: []const u8) MatchError!?Candidate {
        if (try emailKey(a, address, false)) |key| {
            return self.exact_emails.get(key) orelse self.folded_emails.get((try emailKey(
                a,
                address,
                true,
            )).?);
...
pub const PhoneKeyError = u.Allocator.Error || error{InvalidContacts};
pub const ValidateRegionError = u.Allocator.Error || error{InvalidContactsPhoneRegion};
pub const RequestPermissionError = error{
    ContactsPromptTimedOut,
    InstalledAppRequired,
    UnsupportedPlatform,
};
pub const ThumbnailError = std.Io.Dir.ReadFileAllocError || std.json.ParseError(std.json.Scanner);
pub const CompleteError = Journal.QueryError || std.json.ParseError(std.json.Scanner) || error{
    AssetQueueFull,
    InvalidRequest,
...
fn emailKey(a: u.Allocator, address: []const u8, fold_local: bool) !?[]const u8 {
    ...
    if (trimmed.len > 254 or at == 0 or at + 1 == trimmed.len or std.mem.indexOfScalar(
        u8,
        trimmed[at + 1 ..],
        '@',
    ) != null) return null;
...
pub fn phoneKey(a: u.Allocator, address: []const u8, region: []const u8) PhoneKeyError![]const u8 {
    ...
    return std.fmt.allocPrint(
        a,
        "{s}:{s}",
        .{ if (std.mem.startsWith(u8, trimmed, "+") and t.validAddress(trimmed)) "phone" else "exact", trimmed },
    );
}
pub fn validateRegion(a: u.Allocator, region: []const u8) ValidateRegionError!void {
    ...
    if (comptime native) if (zr_contacts_region_valid(try a.dupeZ(u8, region)) == 0) return error.InvalidContactsPhoneRegion;
}
...
pub fn permission() Permission {
    return if (comptime native) permissionCode(zr_contacts_status()) else .unavailable;
}
...
pub fn readerProbeExit() noreturn {
    std.process.exit(if (comptime native) @intCast(zr_contacts_reader()) else 105);
}
pub fn startAuthorizationChecks() void {
    if (comptime native) zr_contacts_monitor_status();
}
pub fn authorizationLoop(core: *Core) void {
    while (!core.stop.load(.acquire)) {
        core.sleep(1000);
        if (comptime native) zr_contacts_refresh_status();
    }
}
...
pub fn currentStatus(status: Status) Status {
    return if (comptime native) withPermission(status, permission()) else status;
}
...
pub const PhotoSource = struct {
    contact_id: []const u8,
    generation: u64,
    token: []const u8,
};
pub fn photoCurrent(a: u.Allocator, core: *Core, photo: PhotoSource) u.Allocator.Error!bool {
    if (comptime native) return permission() == .authorized and zr_contacts_generation() == photo.generation;
    const observed = snapshot(a, core, null, null, true) catch |err| {
        if (err == error.OutOfMemory) return error.OutOfMemory;
        return false;
    };
    return observed.permission == .authorized and !observed.failed and observed.generation == photo.generation;
}
pub fn thumbnail(a: u.Allocator, core: *Core, photo: PhotoSource, output: c_int) ThumbnailError!c_int {
    if (comptime native) return zr_contacts_thumbnail(
        try a.dupeZ(u8, photo.contact_id),
        photo.generation,
        output,
    );
...
fn snapshot(
    ...
    if (comptime native) {
        ...
        const raw = zr_contacts_snapshot(try a.dupeZ(u8, core.contacts_phone_region)) orelse return .{
            .permission = permission(),
            .generation = generation,
            .failed = true,
        };
        defer u.c.free(raw);
        if (zr_contacts_generation() != generation) return .{
            .permission = p,
            .generation = generation,
            .failed = true,
        };
        const contacts = (try std.json.parseFromSlice(
            []const Contact,
            a,
            std.mem.span(raw),
...
fn work(a: u.Allocator, j: Journal) ![]Work {
    const s = try j.db.prepare("SELECT i.record,w.generation FROM identity_work w JOIN identities i ON i.id=w.identity_id ORDER BY w.attempt_ms,i.id LIMIT 100");
    defer s.close();
    var jobs: std.ArrayList(Work) = .empty;
    while (try s.step()) try jobs.append(
        a,
        .{ .value = (try std.json.parseFromSlice(t.Identity, a, s.bytes(0), .{ .allocate = .alloc_always })).value, .generation = s.int(1) },
    );
...
pub fn loop(core: *Core) void {
    ...
    while (!core.stop.load(.acquire)) {
        ...
        if (observed.permission == .authorized and (changed or refresh) and !failed) {
            ...
            const parsed = if (copied) |bytes| std.json.parseFromSlice(
                []const Contact,
...
pub fn complete(
    ...
    try j.begin();
    errdefer j.rollback();
    const s = try j.db.prepare("SELECT generation FROM identity_work WHERE identity_id=?");
    defer s.close();
    ...
    var updated = value;
    if (!failed) updated.avatar = try Assets.avatar(a, j, value.id, photo);
    try j.updateIdentity(a, updated);
    if (!failed) {
        try j.execute("DELETE FROM contact_mappings WHERE identity_id=?", &.{.{ .text = value.id }});
        if (contact_id) |id| try j.execute("INSERT INTO contact_mappings VALUES(?,?,?,?)", &.{
            .{ .text = value.id },
            .{ .text = id },
            .{ .text = source_version },
            .{ .int = normalization_version },
        });
    }
...
test "contact matching prefers exact email local part, preserves aliases, and rejects ambiguous contacts" {
    ...
    const index = try Index.init(a, &.{
        .{
            .id = "unified-a",
            .name = "Élodie 👩‍💻",
            .emails = &.{ "Alice@Example.invalid", "alias+tag@example.invalid" },
            .phones = &.{"phone:+14155550123"},
        },
        .{
            .id = "unified-b",
    ...
    try std.testing.expectEqual(
        @as(usize, 0),
        (try index.match(a, " Alice@EXAMPLE.invalid ", "")).?.index,
    );
...
test "native phone matching uses explicit regions and keeps short codes and extensions exact" {
    if (comptime !native) return error.SkipZigTest;
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const a = arena.allocator();
    try std.testing.expectEqualStrings(
        "phone:+14155550123",
        try phoneKey(a, "(415) 555-0123", "US"),
    );
    try std.testing.expectEqualStrings(
        "phone:+442079460123",
        try phoneKey(a, "020 7946 0123", "GB"),
    );
    try std.testing.expectEqualStrings(
        "phone:+14155550123",
        try phoneKey(a, "+1 (415) 555-0123", ""),
    );
    ...
    try std.testing.expectEqualStrings(
        "exact:+14155550123 ext 9",
        try phoneKey(a, "+14155550123 ext 9", "US"),
    );
    try std.testing.expectError(error.InvalidContactsPhoneRegion, validateRegion(a, "ZZ"));
    try std.testing.expectEqualStrings(
        "Älice@éxample.invalid",
        (try emailKey(a, " Älice@ÉXAMPLE.invalid ", false)).?,
    );
    try std.testing.expectEqualStrings(
        "älice@éxample.invalid",
        (try emailKey(a, "Älice@ÉXAMPLE.invalid", true)).?,
    );
}
...
test "stale contact completions cannot publish across work generations or epoch reset" {
    ...
    try complete(a, j, epoch, 1, v, "private-contact-id", "version", false, null);
    try std.testing.expectEqual(before, try j.sequence());
    try complete(a, j, epoch, 2, v, "private-contact-id", "version", false, null);
    try std.testing.expect((try j.sequence()) > before);
    try std.testing.expectEqual(
        @as(i64, 1),
        try j.db.scalar("SELECT count(*) FROM contact_mappings"),
    );
    for (try j.eventsWithIdentities(a, 0, true)) |frame| try std.testing.expect(std.mem.indexOf(
        u8,
        frame.frame,
        "private-contact-id",
    ) == null);
