...
pub fn observe(s: *const Self) u32 {
    return s.generation.load(.acquire);
}
pub fn notify(s: *Self, io: std.Io) void {
    _ = s.generation.fetchAdd(1, .release);
    io.futexWake(u32, &s.generation.raw, std.math.maxInt(u32));
}
pub fn wait(s: *Self, io: std.Io, observed: u32, milliseconds: i64) void {
    io.futexWaitTimeout(u32, &s.generation.raw, observed, .{ .duration = .{ .raw = .fromMilliseconds(milliseconds), .clock = .awake } }) catch {};
}
