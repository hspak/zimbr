...
fn dropTexture(s: *Self, e: *Entry) void {
    if (e.texture) |tx| {
...
pub fn nextFrame(s: *Self, scale: f32) void {
    if (s.scale != scale) s.deinit();
    s.scale = scale;
    s.frame += 1;
}
fn get(s: *Self, text: []const u8, size: i32, width: f32, single_line: bool, subpixel: bool) !*Entry {
    ...
    const e = Entry{ .hash = key, .text = copy, .layout = layout, .used = s.frame, .fallback = original == null, .width = @as(f32, @floatFromInt(c.zc_text_width(layout))) / s.scale, .height = @as(f32, @floatFromInt(c.zc_text_height(layout))) / s.scale };
...
pub fn lineInkCenterY(s: *Self, text: []const u8, size: i32, width: f32) f32 {
...
fn drawEntry(s: *Self, e: *Entry, x: f32, y: f32, color: rl.Color, start: usize, end: usize, background: ?rl.Color) void {
    ...
    const visible_end: i32 = @intFromFloat(@min(@as(f32, @floatFromInt(full_height)), @ceil((@as(f32, @floatFromInt(rl.getScreenHeight())) - y) * s.scale / 256) * 256));
    ...
    while (top < visible_end) {
        ...
        if (e.texture == null) {
            ...
            e.texture = rl.loadTextureFromImage(.{ .data = pixels, .width = c.zc_text_width(e.layout), .height = tile_height, .mipmaps = 1, .format = .uncompressed_r8g8b8a8 }) catch {
                e.raster_failed = true;
                return;
            };
        ...
        rl.drawTexturePro(tx, .{ .x = 0, .y = 0, .width = @floatFromInt(tx.width), .height = @floatFromInt(tx.height) }, .{ .x = @round(x * s.scale) / s.scale, .y = (@round(y * s.scale) + @as(f32, @floatFromInt(top))) / s.scale, .width = @as(f32, @floatFromInt(tx.width)) / s.scale, .height = @as(f32, @floatFromInt(tx.height)) / s.scale }, .{ .x = 0, .y = 0 }, 0, rl.Color.white);
        top += tile_height;
    }
}
