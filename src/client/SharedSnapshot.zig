...
pub fn create(store: Store, selected: []const u8, generation: u64, previous: ?*Self) !*Self {
    const s = try a.create(Self);
    errdefer a.destroy(s);
    var arena = std.heap.ArenaAllocator.init(a);
    errdefer arena.deinit();
    const epoch = try store.get(arena.allocator(), "epoch");
    const reusable = if (previous) |old| std.mem.eql(u8, old.snapshot.selected, selected) and std.mem.eql(u8, old.snapshot.epoch, epoch) else false;
    const history = try MessageHistory.create(store, selected, if (reusable) previous.?.history else null);
...
pub fn retain(s: *Self) *Self {
    _ = s.refs.fetchAdd(1, .monotonic);
    return s;
}
pub fn release(s: *Self) void {
