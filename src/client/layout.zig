...
pub const Areas = struct { sidebar: rl.Rectangle, header: rl.Rectangle, history: rl.Rectangle, composer: rl.Rectangle, status: rl.Rectangle };
...
pub fn frame(width: f32, height: f32) Areas {
    ...
    clay.UI()(.{ .id = .ID("root"), .layout = .{ .sizing = .grow, .direction = .left_to_right } })({
        clay.UI()(.{ .id = .ID("sidebar"), .layout = .{ .sizing = .{ .w = .fixed(if (width < 950) 270 else 310), .h = .grow } } })({});
        clay.UI()(.{ .layout = .{ .sizing = .grow, .direction = .top_to_bottom } })({
            ...
            clay.UI()(.{ .id = .ID("status"), .layout = .{ .sizing = .{ .w = .grow, .h = .fixed(36) } } })({});
        });
    });
    _ = clay.endLayout();
    return .{ .sidebar = rect("sidebar"), .header = rect("header"), .history = rect("history"), .composer = rect("composer"), .status = rect("status") };
}
