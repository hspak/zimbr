...
pub const Result = struct {
    ...
    state: enum { ready, offline, pending, failed, retired, denied } = .failed,
    retry_at: i64 = 0,
...
pub fn context(s: *Self, epoch: []const u8, chat: []const u8, credentials: u64, online: bool, avatars: bool) !bool {
...
pub fn key(s: *Self, asset: t.AssetRef) [64:0]u8 {
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    for ([_][]const u8{ s.config.relay_url, &s.ca_digest, s.epoch, asset.id, asset.version, @tagName(asset.variant) }) |part| {
        hash.update(part);
        hash.update(&.{0});
    }
...
pub fn request(s: *Self, asset: t.AssetRef) !void {
    s.mutex.lockUncancelable(s.io);
    defer s.mutex.unlock(s.io);
    if (s.epoch.len == 0 or (!s.avatars and asset.variant == .avatar)) return;
    const cache_key = s.key(asset);
    for (s.active) |active| if (active) |r| if (r.generation == s.generation and u.eq(&r.key, &cache_key)) {
        r.wanted_at = u.now();
        return;
    };
    ...
    const owned = try std.json.parseFromSliceLeaky(t.AssetRef, arena.allocator(), try u.json(arena.allocator(), asset), .{});
    const r = try a.create(Request);
    errdefer a.destroy(r);
    r.* = .{ .arena = arena, .asset = owned, .key = cache_key, .generation = s.generation, .wanted_at = u.now() };
    try s.queue.append(a, r);
    s.wake();
}
pub fn take(s: *Self) ?*Result {
    s.mutex.lockUncancelable(s.io);
    defer s.mutex.unlock(s.io);
...
pub fn release(s: *Self, result: *Result) void {
    result.destroy();
    s.mutex.lockUncancelable(s.io);
    s.delivering = false;
    s.mutex.unlock(s.io);
...
fn deliver(s: *Self, request_value: *Request, result: *Result, lane: usize) void {
    s.mutex.lockUncancelable(s.io);
    defer s.mutex.unlock(s.io);
    s.active[lane] = null;
            ...
            for (0..2) |lane| {
                ...
                if (active) |r| {
                    ...
                    if (failure.curl_code == 0 and status == 200) {
                        if (c.zc_image_read(s.dir, &r.temporary, &result.pixels) == 1 and c.zc_cache_install(s.dir, &r.temporary, &r.key, r.fd) == 1) {
                            result.state = .ready;
                            c.zc_cache_prune(s.dir, disk_budget - 16 * 1024 * 1024);
                        } else {
                        ...
                        result.state = .denied;
                        setReason(result, "Image access unavailable · Reconnect after fixing credentials");
                    } else if (status == 409) {
                        var len: usize = 0;
                        const ptr = c.zc_net_media_body(net.?, @intCast(lane), &len);
                        if (ptr != null) {
                            const parsed = std.json.parseFromSlice(struct { retryable: bool = false, retry_after: ?u32 = null, asset: ?t.AssetRef = null }, a, ptr[0..len], .{ .ignore_unknown_fields = true }) catch null;
                            if (parsed) |data| {
                                defer data.deinit();
                ...
                if (net == null) {
                    ...
                    net = c.zc_net_new(s.config.relay_url, s.config.ca_file, s.config.client_cert_file, s.config.client_key_file, null, null, &failure, &identity);
                }
                if (net == null) {
                    result.state = .denied;
                ...
                const path = try std.fmt.allocPrintSentinel(r.arena.allocator(), "/v1/assets/{s}/{s}/{s}", .{ try encode(r.arena.allocator(), r.asset.id), try encode(r.arena.allocator(), r.asset.version), @tagName(r.asset.variant) }, 0);
...
fn encode(ar: u.Allocator, raw: []const u8) ![]const u8 {
    ...
    for (raw) |byte| if (std.ascii.isAlphanumeric(byte) or byte == '-' or byte == '_') {
        ...
        try result.appendSlice(ar, &.{ '%', hex[byte >> 4], hex[byte & 15] });
    };
    return result.items;
}
...
test "media cache validates files and pixels, installs atomically, and evicts private avatars" {
    ...
    const png = [_]u8{ 137, 80, 78, 71, 13, 10, 26, 10, 0, 0, 0, 13, 73, 72, 68, 82, 0, 0, 0, 1, 0, 0, 0, 1, 8, 6, 0, 0, 0, 31, 21, 196, 137, 0, 0, 0, 11, 73, 68, 65, 84, 120, 156, 99, 96, 0, 2, 0, 0, 5, 0, 1, 122, 94, 171, 63, 0, 0, 0, 0, 73, 69, 78, 68, 174, 66, 96, 130 };
...
test "asset keys separate relay identities, epochs, versions and variants" {
    var media = Self{ .io = std.testing.io, .config = .{ .data = "", .relay_url = "https://one.invalid" }, .epoch = "epoch-1" };
    const asset = t.AssetRef{ .id = "asset", .version = "1", .variant = .inline_image };
