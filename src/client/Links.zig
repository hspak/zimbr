...
pub fn parse(a: std.mem.Allocator, value: []const u8) ?Target {
    if (!((value.len >= 7 and std.ascii.eqlIgnoreCase(value[0..7], "http://")) or (value.len >= 8 and std.ascii.eqlIgnoreCase(value[0..8], "https://")))) return null;
    if (value.len == 0 or value.len > 8192 or std.mem.indexOfScalar(u8, value, 0) != null) return null;
...
pub fn open(value: Target) bool {
    return c.zc_url_open(value.url) != 0;
}
