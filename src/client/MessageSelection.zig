...
pub fn clear(s: *MessageSelection) void {
    a.free(s.id);
    a.free(s.preview);
    a.free(s.full);
    s.* = .{};
...
pub fn matches(s: MessageSelection, id: []const u8, pending: bool, preview: []const u8) bool {
    return s.id.len > 0 and s.pending == pending and std.mem.eql(u8, s.id, id) and std.mem.eql(
        u8,
        s.preview,
        preview,
    );
...
pub fn selectAll(s: *MessageSelection) void {
    s.anchor = 0;
...
pub fn selected(s: MessageSelection) []const u8 {
...
test "whole message copy retains text beyond the rendered preview" {
    ...
    try selection.begin(
        ...
        true,
        "Visible\n… Preview shortened",
        "Visible and the rest of the original message",
        0,
    );
