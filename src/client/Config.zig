...
fn preferences(a: u.Allocator, raw: []const u8) !Preferences {
    ...
    for ([_][]const u8{
        ...
        if (value.value.object.contains(field)) {
            log.err(
                "obsolete transport field '{s}'; configure relay_url, ca_file, client_cert_file and client_key_file",
                .{field},
            );
...
pub fn parse(init: std.process.Init) ParseError!Config {
    ...
    for (args[1..]) |arg| {
        ...
        if (u.eq(arg, "--port") or u.eq(arg, "--token-file")) {
            log.err(
                "obsolete transport option '{s}'; use --relay-url, --ca-file, --client-cert-file and --client-key-file",
                .{arg},
            );
            return error.ObsoleteTransportConfiguration;
        }
    ...
    const base = init.environ_map.get("XDG_DATA_HOME") orelse try std.fmt.allocPrint(
        a,
        "{s}/.local/share",
        .{home},
    );
    var data: []const u8 = try std.fmt.allocPrint(a, "{s}/zimbr", .{base});
    const confbase = init.environ_map.get("XDG_CONFIG_HOME") orelse try std.fmt.allocPrint(
        a,
        "{s}/.config",
        .{home},
    );
    ...
    if (read < 0) {
        log.err(
            "unsafe configuration: {s}; use an owned 0600 file in a 0700 directory without symlinks",
            .{conf},
        );
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (u.eq(args[i], "--data-dir")) data = value else if (u.eq(args[i], "--relay-url")) pref.relay_url = value else if (u.eq(
            args[i],
            "--ca-file",
        )) pref.ca_file = value else if (u.eq(
            args[i],
            "--client-cert-file",
        )) pref.client_cert_file = value else if (u.eq(
            args[i],
            "--client-key-file",
        )) pref.client_key_file = value else if (u.eq(
            args[i],
            "--screenshot",
        )) screenshot = try a.dupeZ(
            u8,
            value,
        ) else if (u.eq(
            args[i],
            "--frames",
        )) frames = try std.fmt.parseInt(
            usize,
            value,
            10,
    ...
    if (std.mem.indexOfScalar(u8, origin, 0) != null or c.zc_origin_valid(origin) == 0) {
        log.err(
            "relay_url must be an HTTPS origin, e.g. https://mac-mini.local:8731 (no userinfo, path, query or fragment)",
            .{},
        );
    ...
    for ([_]?[]const u8{
        pref.ca_file,
        pref.client_cert_file,
        pref.client_key_file,
    }) |path| {
        if (path == null or path.?.len == 0 or path.?[0] != '/' or std.mem.indexOfScalar(
            u8,
            path.?,
            0,
        ) != null) {
            log.err("ca_file, client_cert_file and client_key_file must all be absolute paths", .{});
    ...
    return .{
        .data = dir,
        .relay_url = origin,
        .ca_file = try a.dupeZ(u8, pref.ca_file.?),
        .client_cert_file = try a.dupeZ(u8, pref.client_cert_file.?),
        .client_key_file = try a.dupeZ(u8, pref.client_key_file.?),
